...
# Agent Works Here
async def query_retrieval_strategy(QUERY_QUESTION, mistral):
    if mistral:
        client = Mistral(api_key=llm_api_key)
    else:
        client = AsyncOpenAI(
...
async def scoring_alt(QUERY_QUESTION, mistral):
    if mistral:
        client = Mistral(api_key=llm_api_key)
    else:
        client = AsyncOpenAI(
...
if __name__ == '__main__':
    ...
    try:
        retrieval_strategy = asyncio.run(query_retrieval_strategy(QUERY_QUESTION, args.mistral))
