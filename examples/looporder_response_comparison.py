...
try:
    ...
    pass
try:
    os.mkdir(f"{args.query_output_path}/with_multiagent_rag_intermediate")
except:
    pass
try:
...
for filename in file_list:
    ...
    print('# 1. Run query without the use of GraphRAG')
    ...
    print('# 2. Run query with the use of GraphRAG')
    with open(filename, 'r', encoding='utf-8-sig') as file:
        try:
    ...
    print('# 3. Run query with the use of Multi-agent GraphRAG')
    with open(filename, 'r', encoding='utf-8-sig') as file:
        # Read the contents of the file
        try:
        ...
        if args.mistral:
            if args.alternate_agent:
                try:
                    start = time()
                    os.system(f"python ./examples/multi_agent_graphrag_parser.py --work_directory {args.work_directory} --query_input_path {filename} --query_output_path {args.query_output_path}/with_multiagent_rag/response_{os.path.basename(filename)} --mistral --alternate_agent --intermediate_output_path {args.query_output_path}/with_multiagent_rag_intermediate/response_{os.path.basename(filename)}")
                except:
                    start = time()
                    os.system(f"python ./examples/multi_agent_graphrag_parser.py --work_directory {args.work_directory} --query_input_path {filename} --query_output_path {args.query_output_path}/with_multiagent_rag/response_{os.path.basename(filename)} --mistral --alternate_agent --intermediate_output_path {args.query_output_path}/with_multiagent_rag_intermediate/response_{os.path.basename(filename)}")
            else:
                try:
                    start = time()
                    os.system(f"python ./examples/multi_agent_graphrag_parser.py --work_directory {args.work_directory} --query_input_path {filename} --query_output_path {args.query_output_path}/with_multiagent_rag/response_{os.path.basename(filename)} --mistral --intermediate_output_path {args.query_output_path}/with_multiagent_rag_intermediate/response_{os.path.basename(filename)}")
                except:
                    start = time()
                    os.system(f"python ./examples/multi_agent_graphrag_parser.py --work_directory {args.work_directory} --query_input_path {filename} --query_output_path {args.query_output_path}/with_multiagent_rag/response_{os.path.basename(filename)} --mistral --intermediate_output_path {args.query_output_path}/with_multiagent_rag_intermediate/response_{os.path.basename(filename)}")
        else:
            if args.alternate_agent:
                try:
                    start = time()
                    os.system(f"python ./examples/multi_agent_graphrag_parser.py --work_directory {args.work_directory} --query_input_path {filename} --query_output_path {args.query_output_path}/with_multiagent_rag/response_{os.path.basename(filename)} --alternate_agent --intermediate_output_path {args.query_output_path}/with_multiagent_rag_intermediate/response_{os.path.basename(filename)}")
                except:
                    start = time()
                    os.system(f"python ./examples/multi_agent_graphrag_parser.py --work_directory {args.work_directory} --query_input_path {filename} --query_output_path {args.query_output_path}/with_multiagent_rag/response_{os.path.basename(filename)} --alternate_agent --intermediate_output_path {args.query_output_path}/with_multiagent_rag_intermediate/response_{os.path.basename(filename)}")
            else:
                try:
                    start = time()
                    os.system(f"python ./examples/multi_agent_graphrag_parser.py --work_directory {args.work_directory} --query_input_path {filename} --query_output_path {args.query_output_path}/with_multiagent_rag/response_{os.path.basename(filename)} --intermediate_output_path {args.query_output_path}/with_multiagent_rag_intermediate/response_{os.path.basename(filename)}")
                except:
                    start = time()
                    os.system(f"python ./examples/multi_agent_graphrag_parser.py --work_directory {args.work_directory} --query_input_path {filename} --query_output_path {args.query_output_path}/with_multiagent_rag/response_{os.path.basename(filename)} --intermediate_output_path {args.query_output_path}/with_multiagent_rag_intermediate/response_{os.path.basename(filename)}")
        time_taken = time() - start
    ...
    print('# 4. Comparison of the results')
    with open(filename, 'r', encoding='utf-8-sig') as file:
        # Read the contents of the file
        question = file.read()
